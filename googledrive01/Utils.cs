...
namespace googledrive01
{
    public class Utils
    {
        ...
        {
            var provider = new StoredStateClient(GoogleAuthenticationServer.Description, Properties.Settings.Default.CLIENT_ID, Properties.Settings.Default.CLIENT_SECRET, credentials);
        ...
        {
            var provider = new NativeApplicationClient(GoogleAuthenticationServer.Description, Properties.Settings.Default.CLIENT_ID, Properties.Settings.Default.CLIENT_SECRET);
            IAuthorizationState state = new AuthorizationState();
            state.Callback = new Uri(Properties.Settings.Default.REDIRECT_URI);
            try
            {
        ...
        public static String GetAuthorizationUrl(String emailAddress, String state)
        {
            ...
            provider.ClientIdentifier = Properties.Settings.Default.CLIENT_ID;

           // IAuthorizationState authorizationState = new AuthorizationState(new[] { DriveService.Scopes.Drive.GetStringValue() });
            IAuthorizationState authorizationState = new AuthorizationState(new[] { DriveService.Scopes.Drive.GetStringValue() });
           
            authorizationState.Callback = new Uri(Properties.Settings.Default.REDIRECT_URI);
